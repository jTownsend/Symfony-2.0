	...
	$.fn.ptags = function(options) {
		...
		all_elements.each(function() {
			...
			ptags.ptags_add = function(textorarray){
				newTextArray = (typeof textorarray == "string") ? [textorarray] : textorarray;
				
				i = 0;
				$.each(newTextArray, function() {
					newTextArray[i] = $('<div/>').text(""+this).text();
					i++;
				});
				
				$.merge(ptags.ptags_tags, newTextArray);
			...
			ptags.ptags_remove = function(textorarray){
				$.each((typeof textorarray == "string" ? [textorarray] : textorarray), function(){
					var i = $.inArray($('<div/>').text(""+this).html(), ptags.ptags_tags);
					if (i > -1) {
			...
			ptags.ptags_update_tags = function(){
				...
				if (opts.ptags_max_number) {
					counter = $('.ptags-counter');
					counter.text(ptags.count + "/" + opts.ptags_max_number)
					
					if (ptags.count == opts.ptags_max_number) {
						counter.html(counter.html() + " <small>Remove a value before adding more</small>")
			...
			// Add counter
			if (opts.ptags_max_number)
				ptags.ptags_container.prepend($("<span />").addClass("ptags-counter"));
			...
			var input_ptags_delimiter = $('<input type="hidden"></input>').attr('value', opts.ptags_delimiter).attr("name", input_box.attr("name") + "__delimiter");;
			ptags.ptags_container.append(input_ptags_delimiter);
