...
class app_devUrlMatcher extends Symfony\Component\Routing\Matcher\UrlMatcher
{
    ...
    public function match($url)
    {
        ...
        if (0 === strpos($url, '/hello') && preg_match('#^/hello/(?P<name>[^/\.]+?)$#x', $url, $matches)) {
            return array_merge($this->mergeDefaults($matches, array (  '_controller' => 'Application\\JonTestBundle\\Controller\\JonTestController::indexAction',)), array('_route' => 'hello'));
        }

        if (0 === strpos($url, '/dawg') && preg_match('#^/dawg/(?P<name>[^/\.]+?)$#x', $url, $matches)) {
            return array_merge($this->mergeDefaults($matches, array (  '_controller' => 'Application\\JonTestBundle\\Controller\\JonTestController::indexAction',)), array('_route' => 'dawg'));
        }
